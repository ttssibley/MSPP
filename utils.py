...
class ModelSaver:
    ...
    def save_models(self, score, iter_num, model, ious):
...
class LRScheduler:
    ...
    def __init__(self, lr_scheduler_args, optimizer):
        ...
        if self.args is None:
            ...
            return
        if self.args.type not in self.LR_SCHEDULER_MAP:
            raise ValueError(f"unsupported lr scheduler: {self.args.type}")
        else:
            if self.args.type == 'ExponentialLR':
                self.args.params['gamma'] = np.exp(np.log(self.args.decay_rate) / self.args.decay_len)
            self.lr_scheduler = self.LR_SCHEDULER_MAP[self.args.type](
                optimizer, **self.args.params
            )
    ...
    def step(self):
        if self.no_scheduler:
            return
        if self.args.type == 'ExponentialLR':
            if self.iter in range(self.args.decay_start, self.args.decay_start + self.args.decay_len):
